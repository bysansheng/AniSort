...
class AniSort(object):
    ...
    def get_ani_info(self, name: str) -> dict:
        ...
        try:
            res = requests.get(f"https://api.themoviedb.org/3/search/movie", params={
                "query": match[1],
                "language": "zh-CN",
        ...
        return {
            "name": info["title"],
            "date": info["release_date"].split('-')[0] or "年份未知"
        }
    ...
    def parse(self, name: str) -> dict:
        ...
        for p in self.patterns:
            if p["type"] in ["SE_EP", "EP", "Label"] or not(match := p["regex"].search(name)):
                continue
    ...
    def normalize(self, path: Path) -> str:
        ...
        if (parse_info :=  self.parse(path.name)) and parse_info["normalize"]:
            ...
            return f"./{self.ani_name}/" + parse_info["normalize"].replace("S{season}_", '').format(
